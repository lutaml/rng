...
module Rng
  ...
  class SchemaValidator
    RNG_NS = "http://relaxng.org/ns/structure/1.0".freeze
    ANNOTATIONS_NS = "http://relaxng.org/ns/compatibility/annotations/1.0".freeze
    ...
    def validate_node(node, parent_context: nil)
      ...
      if is_root
        ...
        unless ns == RNG_NS || (ns.nil? && !local_name.empty?)
          return
        end
    ...
    def get_attribute_name_class(attr_node)
      name = attr_node["name"]
      if name
        return { type: :name, name: name, ns: attr_node["ns"] || "" }
      end
    ...
    def get_element_name_class(element_node)
      ...
      if name
        return { type: :name, name: name, ns: element_node["ns"] || "" }
      end
    ...
    def name_classes_overlap?(nc1, nc2)
      if nc1[:type] == :name && nc2[:type] == :name
        return nc1[:name] == nc2[:name] && nc1[:ns] == nc2[:ns]
      end
      if nc1[:type] == :anyName && nc2[:type] == :anyName
        return true
      end
      if nc1[:type] == :anyName
        return anyName_overlaps_with?(nc1, nc2)
      end
      if nc2[:type] == :anyName
        return anyName_overlaps_with?(nc2, nc1)
      end
      if nc1[:type] == :nsName && nc2[:type] == :nsName
        return nc1[:ns] == nc2[:ns]
      end
      if nc1[:type] == :nsName && nc2[:type] == :name
        return nc1[:ns] == nc2[:ns]
      end
      if nc2[:type] == :nsName && nc1[:type] == :name
        return nc1[:ns] == nc2[:ns]
      end
      false
    end
    ...
    def validate_obsolete_attrs(node, xpath)
      node.attributes.each do |name, attr|
        ...
        if attr_ns == RNG_NS && OBSOLETE_ATTRS.include?(name)
          report_error("Attribute '#{name}' on '#{node.name}' is obsolete", xpath: xpath)
        end
        if attr_ns.nil? && attr.namespace&.prefix.nil?
          if name == "name" && node.name == "start"
            report_error("Attribute 'name' on 'start' is obsolete", xpath: xpath)
          end
          if name == "global" && node.name == "attribute"
            report_error("Attribute 'global' on 'attribute' is obsolete", xpath: xpath)
          end
          if %w[key keyRef].include?(name) && node.name == "data"
            report_error("Attribute '#{name}' on 'data' is obsolete", xpath: xpath)
          end
        end
      end
    end
    ...
    def validate_ncname(value, context, xpath)
      ...
      if value.include?(":")
        report_error("'#{context}' name '#{value}' must be an NCName (no colon)", xpath: xpath)
      end
      unless valid_ncname_string?(value)
    ...
    def validate_qname(value, context, xpath, node)
      ...
      if value.include?(":")
        ...
        if prefix && !prefix.empty? && !valid_ncname_string?(prefix)
          report_error("'#{context}' QName prefix '#{prefix}' must be an NCName", xpath: xpath)
        end
        ...
        if prefix && !prefix.empty? && !prefix_declared?(prefix, node)
    ...
    def prefix_declared?(prefix, node)
      ...
      current = node
      while current && current.is_a?(Nokogiri::XML::Element)
    ...
    def validate_ncname_strict(node, xpath)
      ...
      return unless cp
      unless valid_name_start_char?(cp)
    ...
    def validate_datatype_library(node, xpath)
      ...
      begin
        ...
        after_scheme = dtl[uri.scheme.length + 1..-1]
        if !after_scheme || after_scheme.empty?
          report_error("datatypeLibrary '#{dtl}' must have a non-empty scheme-specific part", xpath: xpath)
        end
        if uri.fragment || uri.query
          report_error("datatypeLibrary '#{dtl}' must not have fragment or query", xpath: xpath)
        end
    ...
    def validate_container_children(local_name, node, xpath)
      ...
      if local_name == "start" && kids.length > 1
    ...
    def validate_name_class_and_pattern(local_name, node, xpath)
      ...
      unless has_name_class
        report_error("'#{local_name}' must have a name class", xpath: xpath)
      end
    ...
    def validate_content_model(node, xpath, parent_context)
      case node.name
      when "attribute"
    ...
    def validate_attribute_content(node, xpath)
      rng_ns_children(node).each do |child|
        if ATTR_DISALLOWED.include?(child.name)
          report_error("'attribute' content must not contain '#{child.name}' pattern", xpath: xpath)
        end
      end
    end
    def validate_list_content(node, xpath)
      rng_ns_children(node).each do |child|
        if LIST_DISALLOWED.include?(child.name)
          report_error("'list' content must not contain '#{child.name}'", xpath: xpath)
        end
      end
    end
    def validate_data_except_content(node, xpath)
      rng_ns_children(node).each do |child|
        if DATA_EXCEPT_DISALLOWED.include?(child.name)
          report_error("'data/except' content must not contain '#{child.name}'", xpath: xpath)
        end
      end
    end
    def validate_interleave_content(node, xpath)
      kids = rng_ns_children(node)
      if kids.count { |c| c.name == "text" } > 1
        report_error("'interleave' must not contain multiple 'text' patterns", xpath: xpath)
      end
      names = collect_element_names_non_choice(kids)
      dups = names.tally.select { |_, v| v > 1 }.keys
      report_error("'interleave' must not contain overlapping element names: #{dups.join(', ')}", xpath: xpath) unless dups.empty?
    end
    def validate_mixed_content(node, xpath)
      if rng_ns_children(node).any? { |c| c.name == "mixed" }
    ...
    def validate_context(local_name, node, xpath)
      ...
      case local_name
      when "define"
        unless %w[grammar div include].include?(parent_local)
          report_error("'define' is not allowed inside '#{parent_local}'", xpath: xpath)
        end
      when "start"
        unless %w[grammar div].include?(parent_local)
          report_error("'start' is not allowed inside '#{parent_local}'", xpath: xpath)
        end
      when "include"
        unless %w[grammar div].include?(parent_local)
          report_error("'include' is not allowed inside '#{parent_local}'", xpath: xpath)
        end
      end
    end
    ...
    def validate_xmlns_restrictions(local_name, node, xpath)
      ...
      name = node["name"]
      if name && name.strip == "xmlns"
        report_error("Attribute name 'xmlns' is not allowed", xpath: xpath)
      end
      ns = node["ns"]
      if ns == "http://www.w3.org/2000/xmlns" || ns == "http://www.w3.org/2000/xmlns/"
      ...
      children.each do |child|
        ...
        targets.each do |target|
          if parent.name == "anyName" && target.name == "anyName"
            report_error("'anyName/except' must not contain 'anyName'", xpath: xpath)
          end
          if parent.name == "nsName" && target.name == "nsName"
            parent_ns = parent["ns"] || ""
            child_ns = target["ns"] || ""
            if parent_ns == child_ns
              report_error("'nsName/except' must not contain 'nsName' with same namespace", xpath: xpath)
            end
          end
          if parent.name == "nsName" && target.name == "anyName"
            report_error("'nsName/except' containing 'anyName' results in empty name class", xpath: xpath)
          end
        end
      end
    end
    def validate_name_conflict(local_name, node, xpath)
      return unless %w[element attribute].include?(local_name)
      if node["name"] && rng_ns_children(node).any? { |c| c.name == "name" }
    ...
    def validate_group_content(local_name, node, xpath)
      ...
      rng_ns_children(node).each do |child|
        if %w[name anyName nsName].include?(child.name)
          report_error("'group' must not contain a name class '#{child.name}'", xpath: xpath)
        end
      end
    end
    ...
    def validate_unknown_attrs(node, xpath)
      node.attributes.each do |attr_name, attr|
        ...
        attr_ns = attr.namespace&.href
        if attr_ns == RNG_NS
          report_error("Unknown attribute '#{attr_name}' on '#{node.name}'", xpath: xpath) unless known_rng_attr?(node.name, attr_name)
        end
        if attr_ns.nil? && attr.namespace&.prefix.nil?
          report_error("Unknown attribute '#{attr_name}' on '#{node.name}'", xpath: xpath) unless known_unprefixed_attr?(node.name, attr_name)
        end
      end
    end
    ...
    def check_duplicate_attrs_in_children(nodes, xpath)
      ...
      report_error("'element' must not have duplicate attribute names: #{dups.join(', ')}", xpath: xpath) unless dups.empty?
    end
    ...
    def validate_except_not_empty(local_name, node, xpath)
      ...
      parent = node.parent
      if %w[anyName nsName data].include?(parent&.name)
    ...
    def validate_xmlns_in_anyname_attribute(local_name, node, xpath)
      ...
      unless except_covers_xmlns?(except_node)
    ...
    def except_covers_xmlns?(except_node)
      ...
      rng_ns_children(except_node).each do |child|
        ...
        when "name"
          if child.text.strip == "xmlns" && (child["ns"].nil? || child["ns"] == "")
            covers_bare_xmlns = true
          end
        when "choice"
          ...
          rng_ns_children(child).each do |gc|
            ...
            when "anyName"
              ...
              covers_xmlns_ns = true
            when "nsName"
              if (gc["ns"] || "") == "http://www.w3.org/2000/xmlns/"
                covers_xmlns_ns = true
              end
            when "name"
              if gc.text.strip == "xmlns" && (gc["ns"].nil? || gc["ns"] == "")
                covers_bare_xmlns = true
              end
            end
          end
    ...
    def validate_grammar_structure(local_name, node, xpath)
      ...
      unless has_start || has_include || has_define || kids.empty?
    ...
    def validate_define_combine(local_name, node, xpath)
      ...
      defines.group_by { |d| d["name"] }.each do |name, group|
        ...
        if without_combine > 1
          report_error("Multiple 'define' elements with name '#{name}' without 'combine' attribute", xpath: xpath)
        end
      end
    end
    def validate_combine_consistency(local_name, node, xpath)
      ...
      defines.group_by { |d| d["name"] }.each do |name, group|
        next unless group.length > 1
        vals = group.map { |d| d["combine"] }.compact.map(&:strip).reject(&:empty?).uniq
        if vals.length > 1
          report_error("Inconsistent 'combine' values for define '#{name}': #{vals.join(', ')}", xpath: xpath)
        end
      end
    end
    ...
    def validate_nsname_except_rules(local_name, node, xpath)
      ...
      rng_ns_children(node).each do |child|
        ...
        targets.each do |target|
          if parent.name == "nsName" && target.name == "nsName"
            if (parent["ns"] || "") == (target["ns"] || "")
              report_error("'nsName/except' must not contain 'nsName' with the same namespace", xpath: xpath)
            end
          end
          if parent.name == "nsName" && target.name == "anyName"
            report_error("'nsName/except' containing 'anyName' results in empty name class", xpath: xpath)
    ...
    def validate_param_for_builtin_types(local_name, node, xpath)
      ...
      dtl = node["datatypeLibrary"]
      type = node["type"]
      if (dtl.nil? || dtl.empty?) && %w[string token].include?(type)
    ...
    def validate_data_except_strict(node, xpath)
      ...
      rng_ns_children(node).each do |child|
        if child.name == "oneOrMore"
          report_error("'data/except' content must not contain 'oneOrMore'", xpath: xpath)
        end
      end
    end
    ...
    def validate_interleave_attribute_overlap(local_name, node, xpath)
      ...
      attr_names = rng_ns_children(node).select { |c| c.name == "attribute" }.map { |a| a["name"] }.compact
      dups = attr_names.tally.select { |_, v| v > 1 }.keys
      report_error("'interleave' must not contain overlapping attribute names: #{dups.join(', ')}", xpath: xpath) unless dups.empty?
    end
    def validate_interleave_name_class_overlap(local_name, node, xpath)
      return unless local_name == "interleave"
      ncs = rng_ns_children(node).select { |c| c.name == "element" }.map { |e| get_element_name_class(e) }.compact
      ncs.each_with_index do |nc1, i|
        ncs.each_with_index do |nc2, j|
          next if j <= i
    ...
    def validate_element_attribute_overlap(local_name, node, xpath)
      ...
      rng_ns_children(node).each do |child|
        ...
        rng_ns_children(child).each do |gc|
          ...
          ncs = attrs.map { |a| get_attribute_name_class(a) }.compact
          ncs.each_with_index do |nc1, i|
            ncs.each_with_index do |nc2, j|
              next if j <= i
              if name_classes_overlap?(nc1, nc2)
    ...
    def validate_grammar_root_element(local_name, node, xpath)
      ...
      return unless node.parent&.document&.root == node
      kids = rng_ns_children(node)
      if kids.any? { |c| c.name == "element" } && kids.any? { |c| c.name == "start" }
    ...
    def validate_grammar_must_have_start(local_name, node, xpath)
      ...
      unless has_start || has_include
        ...
        if non_div_kids.any? && !non_div_kids.all? { |c| c.name == "define" }
          ...
          if node.parent&.document&.root == node
    ...
    def validate_ref_resolution(local_name, node, xpath)
      ...
      all_refs.each do |ref|
        ...
        unless all_defines.include?(name)
          report_error("'#{ref[:type]}' name '#{name}' has no matching 'define'", xpath: xpath)
        end
      end
    end
    ...
    def validate_recursive_ref(local_name, node, xpath)
      ...
      defines.each do |name, deps|
        if has_cycle?(name, defines, [])
          report_error("'define' name '#{name}' is recursive", xpath: xpath)
        end
      end
    end
    ...
    def validate_builtin_type(local_name, node, xpath)
      ...
      if dtl.nil? || dtl.empty?
        unless %w[string token].include?(type)
    ...
    def validate_data_except_content_types(local_name, node, xpath)
      ...
      rng_ns_children(node).each do |child|
        if child.name == "empty"
          report_error("'data/except' must not contain 'empty'", xpath: xpath)
        end
      end
    end
    ...
    def validate_infinite_attribute_name_class(local_name, node, xpath)
      ...
      if nc
        ...
        if nc[:type] == :anyName && !nc[:except]
          report_error("'attribute' with 'anyName' (no except) matches infinite names", xpath: xpath)
        end
        # nsName with ns="" is equivalent to anyName
        if nc[:type] == :nsName && (nc[:ns] == "" || nc[:ns].nil?)
          report_error("'attribute' with 'nsName' ns='' matches all names", xpath: xpath)
        end
      end
      ...
      rng_ns_children(node).each do |child|
        ...
        when "name"
          if child.text.strip == "xmlns"
            ns = child["ns"] || ""
            if ns == ""
              report_error("Attribute name class contains 'xmlns'", xpath: xpath)
            end
          end
        when "choice"
          rng_ns_children(child).each do |gc|
        ...
        when "nsName"
          ...
          if exc
            check_name_class_for_xmlns(exc, xpath)
          end
        end
      end
    end
    ...
    def check_start_element_overlap(pattern, xpath)
      ...
      when "group"
        ...
        dups = names.tally.select { |_, v| v > 1 }.keys
        unless dups.empty?
          report_error("'start' group must not have overlapping element names: #{dups.join(', ')}", xpath: xpath)
        end
      when "choice"
      ...
      when "oneOrMore"
        ...
        rng_ns_children(pattern).each do |c|
          ...
          if c.name == "element"
            report_error("'start' oneOrMore of element allows multiple matches", xpath: xpath)
          end
        end
    ...
    def check_start_element_overlap_group(parent, children, xpath)
    ...
    def validate_oneOrMore_attribute_overlap(local_name, node, xpath)
      ...
      rng_ns_children(node).each do |child|
        ...
        report_error("'#{local_name}' with '#{child.name}' containing multiple attributes creates name overlap", xpath: xpath)
      end
    end
    ...
    def check_choice_for_infinite_attr_in_repeat(choice_node, xpath)
      rng_ns_children(choice_node).each do |gc|
        ...
        when "nsName"
          ns = gc["ns"] || ""
          if ns == ""
            report_error("Repeating attribute with choice containing 'nsName' ns='' creates infinite matches", xpath: xpath)
          end
        end
      end
    end
    ...
    def check_choice_for_infinite_attr_name_class(choice_node, xpath)
      rng_ns_children(choice_node).each do |gc|
        ...
        when "nsName"
          ns = gc["ns"] || ""
          if ns == ""
            report_error("'attribute' choice contains 'nsName' ns='' (infinite name class)", xpath: xpath)
          end
        end
      end
    end
