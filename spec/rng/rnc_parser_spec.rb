  ...
  describe '#to_rnc' do
    ...
    context 'with occurrence markers on bare ref patterns' do
      ...
      it 'emits a bare <ref> when there is no occurrence marker' do
        element = root_element('item')
        expect(element.ref.map(&:name)).to eq(['item'])
      ...
      it 'wraps ref+ in <oneOrMore>' do
        element = root_element('item+')
        expect(element.oneOrMore.length).to eq(1)
        expect(element.oneOrMore.first.ref.map(&:name)).to eq(['item'])
        expect(element.ref).to be_empty
      end

      it 'wraps ref? in <optional>' do
        element = root_element('item?')
        expect(element.optional.length).to eq(1)
        expect(element.optional.first.ref.map(&:name)).to eq(['item'])
        expect(element.ref).to be_empty
      end
    end
